  ...
  if [ "$BuildTarget" == "mac" ]; then
    ...
    if [ "$NotarizeRequestId" == "" ] || [ "$NotarizeRequestId" == "go" ]; then
      if [ "$AlphaVersion" == "0" ]; then
        ...
        # AyuGram: inject Gatekeeper bypass helper for unsigned builds
        if [ -f "$FullScriptPath/mac/Install.command" ]; then
            cp "$FullScriptPath/mac/Install.command" "$TempDiskPath/"
            chmod +x "$TempDiskPath/Install.command"
        fi
